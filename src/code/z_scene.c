...
#else
void func_80098904(GlobalContext* globalCtx, SceneCmd* cmd);
#pragma GLOBAL_ASM("asm/non_matchings/code/z_scene/func_80098904.s")
#endif
...
void func_80098D80(GlobalContext* globalCtx, SceneCmd* cmd)
{
    ...
    globalCtx->unk_10A28 = -(Math_Sins(gSaveContext.day_time - 0x8000) * 120.0f) * 25.0f;
    globalCtx->unk_10A2C = (Math_Coss(gSaveContext.day_time - 0x8000) * 120.0f) * 25.0f;
    globalCtx->unk_10A30 = (Math_Coss(gSaveContext.day_time - 0x8000) * 20.0f) * 25.0f;
        ...
        (gSaveContext.entrance_index == 0x0604))
    {
        ...
        gSaveContext.environment_time = gSaveContext.day_time;
        if ((dayTime >= 0x2AAC) && (dayTime < 0x4555))
            gSaveContext.environment_time = 0x3556;
        else if ((dayTime >= 0x4555) && (dayTime < 0x5556))
            gSaveContext.environment_time = 0x5556;
        else if ((dayTime >= 0xAAAB) && (dayTime < 0xB556))
            gSaveContext.environment_time = 0xB556;
        else if ((dayTime >= 0xC001) && (dayTime < 0xCAAC))
            gSaveContext.environment_time = 0xCAAC;
    }
}
#else
void func_80098D80(GlobalContext* globalCtx, SceneCmd* cmd);
#pragma GLOBAL_ASM("asm/non_matchings/code/z_scene/func_80098D80.s")
#endif
