...
void isPrintfInit()
{
    ...
    osEPiWriteIo(sISVHandle, &gISVDbgPrnAdrs->start, NULL);
    osEPiWriteIo(sISVHandle, &gISVDbgPrnAdrs->pos, NULL);
...
u32 is_proutSyncPrintf(void* arg0, const char *str, s32 count)
{
    ...
    if (data != ASCII_TO_U32('I','S','6','4'))
        return 1;
    osEPiReadIo(sISVHandle, &gISVDbgPrnAdrs->pos, &data);
    pos = data;
    osEPiReadIo(sISVHandle, &gISVDbgPrnAdrs->start, &data);
    start = data;
    end = start + count;
    if (end >= 0xffe0)
    {
    ...
    osEPiWriteIo(sISVHandle, &gISVDbgPrnAdrs->start, start);
    return 1;
}
#else
