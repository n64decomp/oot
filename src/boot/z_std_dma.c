...
s32 sDmaMgrQueueFullLogged = 0;

extern u32 D_80013900;
...
s32 DmaMgr_DMARomToRam(u32 rom, u32 ram, u32 size)
{
    ...
    while (size > buffSize)
    {
        ...
        if (D_80009460 == 10)
            osSyncPrintf("%10lld ノーマルＤＭＡ %08x %08x %08x (%d)\n", OS_CYCLES_TO_USEC(osGetTime()), ioMsg.dramAddr, ioMsg.devAddr, ioMsg.size, ((struct_800138F8*)&D_800138F8)->unk_08);
        ...
        if (D_80009460 == 10)
            osSyncPrintf("%10lld ノーマルＤＭＡ START (%d)\n", OS_CYCLES_TO_USEC(osGetTime()), ((struct_800138F8*)&D_800138F8)->unk_08);
...
s32 DmaMgr_DmaCallback0(OSPiHandle* pihandle, OSIoMesg* mb, s32 direction)
{
    ...
    if (D_80009460 == 10)
        osSyncPrintf("%10lld サウンドＤＭＡ %08x %08x %08x (%d)\n", OS_CYCLES_TO_USEC(osGetTime()), mb->dramAddr, mb->devAddr, mb->size, D_80013900);
...
void DmaMgr_Error(DmaRequest* req, const char* file, const char* errorName, const char* errorDesc)
{
    ...
    if (req->filename)
        snprintf(buff1, "DMA ERROR: %s %d", req->filename, req->line);
    else if (sDmaMgrCurFileName)
        snprintf(buff1, "DMA ERROR: %s %d", sDmaMgrCurFileName, sDmaMgrCurFileLine);
    else
        snprintf(buff1, "DMA ERROR: %s", errorName ? errorName : "???");
    
    snprintf(buff2, "%07X %08X %X %s", vrom, ram, size, file ? file : "???");
...
const char* DmaMgr_GetFileNameImpl(u32 vrom)
{
    ...
    //BUG (useless) : since the devs forgot to return in case the file isn't found, the return value will be a pointer to the end of gDmaDataTable
}
        ...
        if (vrom >= iter->vromStart && vrom < iter->vromEnd)
        {
            if (iter->romEnd == 0)
            {
                if (iter->vromEnd < vrom + size)
                    DmaMgr_Error(req, filename, "Segment Alignment Error", "セグメント境界をまたがってＤＭＡ転送することはできません");
            ...
            else
            {
                ...
                if (vrom != iter->vromStart)
                    DmaMgr_Error(req, filename, "Can't Transfer Segment", "圧縮されたセグメントの途中からはＤＭＡ転送することはできません");
                if (size != iter->vromEnd - iter->vromStart)
                    DmaMgr_Error(req, filename, "Can't Transfer Segment", "圧縮されたセグメントの一部だけをＤＭＡ転送することはできません");

                osSetThreadPri(NULL, 0x0A);
                func_80001E14(romStart, ram, romSize);
...
{
    if ((((((ram == 0) || (osMemSize < ram + size + 0x80000000)) || (vrom & 1)) || (vrom > 0x4000000U)) || (size == 0)) || (size & 1))
    ...
    if (sDmaMgrQueueFullLogged == 0 && sDmaMgrMsgQueue.validCount >= sDmaMgrMsgQueue.msgCount)
    {
        ...
        SyncPrintfWithThreadId("../z_std_dma.c", 952);
...
//close from matching, 2 cases of small reordering
#ifdef NON_MATCHING
void DmaMgr_Start()
{
    const char** name;
    ...
    while(iter->vromEnd != 0)
    {
        ...
        if (iter->romEnd != 0)
            decSize = iter->romEnd - iter->romStart;
        else
            decSize = iter->vromEnd - iter->vromStart;

        encSize = (iter->romEnd) ? iter->romEnd - iter->romStart : 0;

        osSyncPrintf("%3d %08x %08x %08x %08x %08x %c %s\n", idx, iter->vromStart, iter->vromEnd, iter->romStart, iter->romEnd, decSize, (encSize > 0x10000) ? '*' : ' ', name ? *name : "");
...
s32 DmaMgr_SendRequest2(DmaRequest* req, u32 ram, u32 vrom, u32 size, u32 unk5, OSMesgQueue* queue, u32 unk7, const char* file, s32 line)
{
    req->filename = file;
    req->line = line;
    DmaMgr_SendRequestImpl(req, ram, vrom, size, unk5, queue, unk7);
}
