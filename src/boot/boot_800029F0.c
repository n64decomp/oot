...
float func_800029F0(const char* exp, s32 arg1, const char* var1Name, float var1, const char* var2Name, float var2, const char* var3Name, float var3)
{
...
s32 func_80002A88(const char* exp, s32 arg1, const char* var1Name, s32 var1, const char* var2Name, s32 var2, const char* var3Name, s32 var3)
{
    if (var1 < var2 || var3 < var1)
...
void func_80002B10(void* ptr, s32 size0)
{
    u8* addr = (u8*)ptr;
    s32 size = (s32)size0;
    s32 rest;
...
void func_80002CB4(s32 value, u32 max, void* ptr, const char *name, const char *file, s32 line)
{
    osSyncPrintf(VT_COL(RED, WHITE) "%s %d %s[%d] max=%u ptr=%08x\n" VT_RST, file, line, name, value, max, ptr);
}

void func_80002D04(const char* name, s32 value, s32 unk, const char* file, s32 line)
{
    u32 mask = (unk - 1); 
    if (value & mask)
...
void NullPointerCheck(const char* exp, void* ptr, const char* file, s32 line)
{
    if (!ptr)
        osSyncPrintf(VT_COL(RED, WHITE) "%s %d:%s は はヌルポインタです\n" VT_RST, file, line, exp);
}

void func_80002DA0(const char* exp, void* ptr0, const char* file, s32 line)
{
    u32 ptr = (u32)ptr0;
...
//there's probalby a macro like this : MACRO(exp) SyncPrintfWithThreadId(__FILE__, __LINE__); osSyncPrintf(exp)
void SyncPrintfWithThreadId(const char* name, s32 line)
{
    osSyncPrintf("<%d %s %d>", osGetThreadId(NULL), name, line);
}

void func_80002E50(const char* name, s32 line)
{
