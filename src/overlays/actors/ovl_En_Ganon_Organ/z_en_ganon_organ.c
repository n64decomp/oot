...
Gfx* func_80A280BC(GraphicsContext* gfxCtx, BossGanon* dorf)
{
    ...
    gDPSetRenderMode(displayListHead++, AA_EN | Z_CMP | IM_RD | CLR_ON_CVG |
            CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | G_RM_FOG_SHADE_A, G_RM_AA_ZB_XLU_SURF2);
...
void EnGanonOrgan_Draw(EnGanonOrgan* this, GlobalContext* globalCtx)
{
    ...
    func_800C6AC4(&gfxArr, globalCtx->gfxCtx, "../z_en_ganon_organ.c", 205);
    osSyncPrintf("ORGAN DRAW  1\n");
    func_80093D18(globalCtx->gfxCtx);
    if ((this->actor.params == 1) && (dorf->organFadeTimer != 0xff))
    {
        gSPSegment(gfxCtx->polyOpa.p++, 0x08, func_80A280BC(globalCtx->gfxCtx, dorf));
        gSPSegment(gfxCtx->polyOpa.p++, 0x09, func_80A28148(globalCtx->gfxCtx, dorf));
    }
    else
    {
        gSPSegment(gfxCtx->polyOpa.p++, 0x08, EnGanonOrgan_EmptyDList(globalCtx->gfxCtx));
        gSPSegment(gfxCtx->polyOpa.p++, 0x09, EnGanonOrgan_EmptyDList(globalCtx->gfxCtx));
    }
    Matrix_Translate(0.0f, 0.0f, 0.0f, 0);
    gSPMatrix(gfxCtx->polyOpa.p++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_ganon_organ.c", 221),
                G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

    gSPDisplayList(gfxCtx->polyOpa.p++, &D_80A2CCA8);
    gSPDisplayList(gfxCtx->polyOpa.p++, &D_80A2EAB0);
    osSyncPrintf("ORGAN DRAW  2\n");
    func_800C6B54(gfxArr, globalCtx->gfxCtx, "../z_en_ganon_organ.c", 230);
}
